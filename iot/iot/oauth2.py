...
def authorize(*args, **kwargs):
	...
	success_url = request_url.scheme + "://" + request_url.netloc + "/api/method/iot.iot.oauth2.approve?" + params
	failure_url = frappe.form_dict["redirect_uri"] + "?error=access_denied"

	if frappe.session['user']=='Guest':
		#Force login, redirect to preauth again.
		frappe.local.response["type"] = "redirect"
		frappe.local.response["location"] = "/login?redirect-to=/api/method/iot.iot.oauth2.authorize?" + quote(params.replace("+"," "))
